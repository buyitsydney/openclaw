...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `scripts/test-feishu-im-chat-permissions.sh` | 全量 `im:chat*` 权限实验（读取 `docker/users.csv`，输出报告到 `tmp/feishu-im-chat-permissions/`） | 已验证可创建群、改群、成员增删、管理员、标签页、Pin/Unpin 等能力，形成 PASS/FAIL/SKIP 报告 | 部分接口属于 `SKIP_UNMAPPED`（官方路径/参数映射未稳定）；`im:chat.announcement:read` 存在 `232097`（docx 公告类型限制） |
