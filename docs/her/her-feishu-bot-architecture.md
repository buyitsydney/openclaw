...
### 组织协作规范（2026-02-27 新增，强制执行）

为避免“用户私有任务 bot 不可见”导致的协作混乱，企业协作统一采用 **Bot-first** 模式：

1. **永远由各自 Her（bot）创建任务**，禁止依赖“用户手工先建任务，再让 bot 接管”。
2. **分配任务必须显式设置 assignee**（被分配人 open_id），否则会出现“任务创建成功但对目标人不可见”。
3. **跨人协作统一在 bot 可见 tasklist 中进行**，禁止把生产协作流程建立在个人私有 tasklist 上。
4. **A 给 B 派任务的标准路径**：A 对自己的 Her 下指令 -> Her 创建 task 并将 B 设为 assignee -> 必要时挂到协作 tasklist。
5. **bot 不作为“读取所有人私有待办”的全局代理**；若需读取/管理，必须先满足该任务/清单对 bot 可见。

该规范适用于个人 Her 与企业 Her，作为默认流程长期执行。
...
5. 回归消息能力：私聊/群聊回复、`/voice`、CardKit 流式、群聊归档保持不变。
6. 若出现异常，优先下线 task 工具注册，保持网关与消息主链路继续可用。
...
### 访问控制架构设计（2026-03-20 更新）

#### 飞书龙虾（@larksuite/openclaw-lark）的实现与效果
...
**龙虾的安全模型**：后台不全员开放（只有主人可见）+ 代码双保险（dmPolicy + groupSenderAllowFrom）。即使后台误配为全员可见，代码层面也能拦住非主人。
...
- [ ] **P0**: gateway.ts 加私聊 DM sender 检查（读 `dm.allowFrom`，不在列表则拒绝）
- [ ] **P1**: 支持 `requireMention` 可配 + `groupSenderAllowFrom`
- [ ] **P1**: Config 字段对齐 upstream 命名（`dmPolicy`、`allowFrom`、`groupPolicy`）
...
9. **上线策略（2026-02-27 新增）**

- 建议先灰度上线（单账号 24h）再全量。
- 灰度期间重点监控：`1470500`、`99992402`、`99991672`。

### 重要 TODO（下次实现，P0）：企业 Her 互拉群产品化

> 本次不新增“互拉群新功能”代码，先上线当前已实现能力；以下内容作为下个迭代的确定性实现清单。

#### 已验证事实（2026-03-03）

- **可行能力**：`POST /im/v1/chats/{chat_id}/members?member_id_type=app_id` 能把另一台 Her（bot）拉入群（`code=0`）
- **验真方式**：`GET /im/v1/chats/{chat_id}` 中 `bot_count` 增加到 `2`，可作为成功判据
- **平台边界**：`GET /im/v1/chats/{chat_id}/members?member_id_type=app_id` 返回 `99992402 field validation failed`
- **列表局限**：`member_id_type=open_id` 的成员列表只返回人类成员，不返回 bot，不能用于“自动发现目标 bot”

#### 下次实现范围（必须全部满足）

1. 增加企业 bot 台账（`bot_name -> app_id`）作为唯一真相，禁止运行时猜测
2. 提供显式“按 `app_id` 拉 bot 入群”能力，失败即返回错误，不做 fallback
3. 增加双向健康探针：`A -> B`、`B -> A` 均需 `add code=0` 且 `bot_count>=2`
4. 发布门禁：任一方向失败即禁止上线

#### 测试脚本与证据（当前阶段）

| 脚本                                         | 用途                                                                                              | 证明了什么                                                                                                          | 当前暴露的错误/边界                                                                                                                    |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/test-feishu-im-chat-permissions.sh` | 全量 `im:chat*` 权限实验（读取 `docker/users.csv`，输出报告到 `tmp/feishu-im-chat-permissions/`） | 已验证可创建群、改群、成员增删、管理员、标签页、Pin/Unpin、`top_notice put/delete` 等能力，形成 PASS/FAIL/SKIP 报告 | `im:chat.announcement:read` 存在 `232097`（docx 公告类型限制）；`tab_type=doc` 仍可能报 `tab content doc field is empty`（需单独修复） |
| `scripts/test-feishu-create-chat.sh`（无密） | 单次快速探针（建群/拉人/发消息/撤回）                                                             | 可快速确认核心链路通断；用于上线前 smoke test                                                                       | 必须通过 `--user-id` 从 `docker/users.csv` 读取凭据；若 `users.csv` 行缺失/字段为空会直接失败                                          |

#### 线上观察口径（避免误判）

- `99992361 open_id cross app`：通常是历史投递队列中的旧 `open_id` 或跨应用 ID，不等于“互拉群能力缺失”
- `99992402 field validation failed`（`members/list` + `app_id`）：是成员列表接口限制，不等于“不能拉 bot”
- 互拉群能力是否正常，只看：`add members(app_id)` 返回码 + `chat_info.bot_count`
