...
- **日程冲突拦截**：导航前主动查 cron 提醒，有冲突时先告知用户再行动 ✅
...
用户: "带老婆去XX饭店吃晚饭"
  ↓
Step 1: Her → openclaw_help("导航预检：用户要去XX饭店，带老婆吃晚饭")
  ↓
OpenClaw: 查 memory（确认地名）+ 查 cron（检查日程冲突）
  → 返回："XX饭店地址是…，近期无冲突"  或  "注意：用户7点有约会，可能冲突"
  ↓
Her 判断：
  - 无冲突 → 进入 Step 2
  - 有冲突 → 告知用户，等用户决定
  ↓
Step 2: Her → car_control(start_navigation) → 车端执行导航 → "好的，开始导航"
  ↓ (静默，用户无感知)
Step 3: Her → openclaw_help("请写入用户memory记录：…")
  ↓
OpenClaw → edit memory → 返回"已记录"或 NO_REPLY
```
...
用户在车上说"导航去北京"，但其实当晚7点有"和老王吃饭"的提醒。如果 Her 直接启动导航，事后才发现冲突，体验极差（"已导航去北京"→"等等您有约会"）。

真正的秘书应该在出发前主动查日程，有冲突第一时间拦截。

### 设计：统一导航预检

将原来的"先查后导"（模糊地名查询）和"日程冲突检查"合并为一个统一的**导航预检**步骤，对所有导航请求生效。
...
每次导航多一次 OpenClaw 调用（预检），增加约 2-5 秒延迟。Live 用过渡语（"好的，我先帮您确认一下"）自然衔接，用户体验可接受。
...
- 二次冲突：用户在高速上说"设下午2点上海的约会"，系统识别时空矛盾并提醒 ✅
- 用户改计划导航回家 → 设置新提醒 → 查询提醒列表，全部正确 ✅

### 扩展性

未来任何需要"预检"的场景（加油前检查日程、接人前确认航班到达时间等）只需在 prompt 中描述预检触发条件，后端零代码修改。
