...
#### 已验证不可行的补充方案：`summary.content` 回读（2026-03-15 实测）

v2 卡片的 `config.summary.content` 字段用于自定义"聊天栏消息预览文本"。**实测结果：不可行。**

实验方法：创建带 `summary.content` 的 v2 CardKit 卡片（card_id `7617350262504442841`），发送到 test 群（`om_x100b54507590813cb3374238046e9ac`），通过 `im.message.get` 回读。

实验结果：

- `im.message.get` 返回的 content 与不带 summary 的 v2 卡片**完全相同**
- 回读内容固定为：占位图 `img_v3_02ad_...` + `"请升级至最新版本客户端，以查看内容"` + 空文本节点
- `summary.content` 字段**不出现在回读结果中**
- **唯一可读字段是 `title`**（header.title.content）

| 卡片类型             | 回读 title | 回读 body                    | 回读 summary |
| -------------------- | ---------- | ---------------------------- | ------------ |
| V1 inline card       | null       | 降级 2D 数组，文本完整可提取 | N/A          |
| V2 CardKit           | ✓ 可读     | "请升级至最新版本客户端"     | **不可见**   |
| V2 + summary.content | ✓ 可读     | "请升级至最新版本客户端"     | **不可见**   |

**结论：`im.message.get` 对 v2 卡片的降级是彻底的 — body 和 summary 都被替换为兜底内容，只有 title 幸存。本地缓存是唯一可行的回读方案。**
...
| summary.content 不可读   | **已验证不可行**（2026-03-15 实测确认） | 主方案（本地缓存）不受影响                         |
