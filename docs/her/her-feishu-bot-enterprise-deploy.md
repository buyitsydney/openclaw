...
通过 API（`feishu_doc` 的 `write`/`append`）写入包含表格的 Markdown 内容时，表格会被静默丢弃。更严重的是，`write` 操作会先清空文档再写入 — 如果写入不完整，文档就被"搞坏"了。
...
飞书官方文档明确说：`convert` 输出的表格块中包含 `merge_info` 只读字段，在调用创建嵌套块之前**必须去除**，否则会报错。当前代码没有做这个处理。
...
3. 新增 `merge_info` 字段清除
4. 适配请求体格式（`{ children }` → `{ children_id, descendants }`）
5. 超过 1000 块时分批调用

**影响范围**：仅 `write` 和 `append` 操作，不影响 `read`/`list_blocks` 等读取操作。
