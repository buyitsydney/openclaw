/**
 * Fill context to trigger auto-compaction on Nova instance (port 19005).
 * Uses the compact-test-send protocol but sends many rounds automatically.
 */
import { randomUUID } from "node:crypto";
import { WebSocket } from "ws";

const GW_URL = process.env.GW_URL || "ws://127.0.0.1:19005/ws?auth=e3f09dbf05db386da4db25659cab33c57648c0c86f46349d";
const SESSION_KEY = `compaction-e2e-${Date.now()}`;
const TOTAL_ROUNDS = Number(process.env.TOTAL_ROUNDS || "15");
const ORIGIN = "http://localhost:19005";
...
ws.on("open", async () => {
  try {
    ...
    const hello = await request("connect", {
      ...
      auth: { token: "e3f09dbf05db386da4db25659cab33c57648c0c86f46349d" },
      minProtocol: 3,
      maxProtocol: 3,
...
ws.on("message", (raw) => {
  ...
  if (frame.type === "res") {
    ...
    // Debug: log lifecycle events
    if (frame.event === "agent" && frame.payload?.stream === "lifecycle") {
      const p = frame.payload;
      console.log(`  [lifecycle] sessionKey=${p.sessionKey} runId=${p.runId} phase=${p.data?.phase}`);
    }

    // Wait for run completion — Nova 0414 protocol:
    //   payload.stream === "lifecycle", payload.data.phase === "end"
    //   payload.sessionKey === "agent:main:<SESSION_KEY>"
    //   payload.runId must not start with "active-memor"
    if (
      waitingForCompletion &&
      frame.event === "agent" &&
      frame.payload?.stream === "lifecycle" &&
      frame.payload?.data?.phase === "end" &&
      frame.payload?.sessionKey?.endsWith(SESSION_KEY) &&
      !String(frame.payload?.runId ?? "").startsWith("active-memor")
    ) {
...
// 60 min timeout
setTimeout(() => {
  console.log(`[timeout] 3600s — completed ${completedRounds} rounds`);
  ws.close();
  process.exit(0);
}, 3600_000);
