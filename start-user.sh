...
#   Global layer: ~/.openclaw/skills/ → all bots, real-time (bind mount)
#   Personal layer: /data/.openclaw/workspace/.agents/skills/ → per-bot (persistent volume)
SHARED_SKILLS_DIR="$HOME/.openclaw/skills"
mkdir -p "$SHARED_SKILLS_DIR"
echo -e "${GREEN}  ✓ 全员 skills: ${SHARED_SKILLS_DIR}${NC}"

# A2A Gateway plugin (always enabled, like feishu)
...
if [ -d "$A2A_PLUGIN_DIR" ] && [ -f "$A2A_PLUGIN_DIR/index.ts" ]; then
  ...
  if [ "${A2A_OUTBOUND:-0}" = "1" ]; then
    echo -e "${GREEN}  ✓ A2A outbound: 已启用${NC}"
  else
    echo -e "  · A2A outbound: 仅接收"
  fi
else
  A2A_PLUGIN_DIR=""
  echo -e "  · A2A plugin: 未安装"
fi
# Skills are synced by deploy-skills.sh (run once per server, not per container).
