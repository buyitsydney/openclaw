...
| `peers[].name`         | string | _required_ | 피어 표시 이름         |
| `peers[].agentCardUrl` | string | _required_ | 피어의 Agent Card URL  |
...
| `routing.rules[].name`           | string | _required_ | 규칙 이름                                   |
...
| `routing.rules[].target.peer`    | string | _required_ | 라우팅할 피어                               |
...
| ------------------------------ | ------ | --------------------------------------------------- |
| `/.well-known/agent-card.json` | GET    | Agent Card 검색 _(별칭: `/.well-known/agent.json`)_ |
