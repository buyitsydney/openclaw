...
export class PeerHealthManager {
  ...
  isAvailable(peerName: string): boolean {
    ...
    if (!state) {
      return true;
    } // Unknown peer → allow (fail at send)
    ...
    if (state.circuit === "open") {
      ...
      if (state.lastFailureAt && Date.now() - state.lastFailureAt >= this.cbConfig.resetTimeoutMs) {
  ...
  recordFailure(peerName: string): void {
    ...
    if (state.circuit === "closed" && state.consecutiveFailures >= this.cbConfig.failureThreshold) {
      state.circuit = "open";
