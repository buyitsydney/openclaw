...
export class MdnsResponder {
  ...
  start(): void {
    if (!this.config.enabled || this.mdnsInstance) {
      return;
    }
    ...
    this.mdnsInstance.on(
      "query",
      (query: { questions?: Array<{ name: string; type: string }> }) => {
        const questions = query.questions || [];
        const isForUs = questions.some(
          (q) => q.name === this.config.serviceName || q.name === this.instanceFullName(),
        );
        if (isForUs) {
          this.advertise();
        }
...
export function buildMdnsAdvertiseConfig(input: MdnsAdvertiseInput): MdnsAdvertiseConfig {
  ...
  const ttl =
    typeof raw.ttl === "number" && Number.isFinite(raw.ttl) && raw.ttl >= 10
      ? raw.ttl
