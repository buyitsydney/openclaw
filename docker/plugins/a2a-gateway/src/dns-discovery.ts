 ...
 */
export function parseTxtRecords(records: string[][]): Map<string, string> {
 ...
 */
export function buildAgentCardUrl(host: string, port: number, pathFromTxt?: string): string {
...
export function discoveredPeerToConfig(peer: DiscoveredPeer): PeerConfig {
  ...
  if (peer.auth?.type && peer.auth?.token) {
    const authType =
      peer.auth.type === "bearer" || peer.auth.type === "apiKey" ? peer.auth.type : "bearer";
...
export class DnsDiscoveryManager {
  ...
  private async discover(): Promise<void> {
    ...
    const existingByKey = new Map(this.discoveredPeers.map((p) => [`${p.host}:${p.port}`, p]));
    ...
    this.discoveredPeers = allPeers.filter((p) => now - p.discoveredAt < p.ttl * 1000);
  ...
  private evictExpired(): void {
    ...
    this.discoveredPeers = this.discoveredPeers.filter((p) => now - p.discoveredAt < p.ttl * 1000);
...
export function parseDnsDiscoveryConfig(
  ...
  return {
    ...
    refreshIntervalMs:
      ...
      Number.isFinite(raw.refreshIntervalMs) &&
      raw.refreshIntervalMs >= 1000
        ? raw.refreshIntervalMs
        : DNS_DISCOVERY_DEFAULTS.refreshIntervalMs,
    mergeWithStatic:
      typeof raw.mergeWithStatic === "boolean"
        ? raw.mergeWithStatic
