...
export class OpenClawAgentExecutor implements AgentExecutor {
  ...
  async cancelTask(taskId: string, eventBus: ExecutionEventBus): Promise<void> {
    ...
    const sessionKey = `agent:${this.defaultAgentId}:a2a:${contextId}`;
  ...
  private async dispatchViaGatewayRpc(
    ...
    try {
      ...
      this.api.logger.info(
        `a2a-gateway: dispatch accountId=${this.ownerAccountId ?? "NONE"} sessionKey=${sessionKey}`,
      );
