...
export function parseRoutingRules(raw: unknown): RoutingRule[] {
  ...
  for (const entry of raw) {
    ...
    const matchRaw =
      obj.match && typeof obj.match === "object" ? (obj.match as Record<string, unknown>) : null;
    if (!matchRaw) {
      continue;
    }
    ...
    const tags = Array.isArray(matchRaw.tags)
      ? (matchRaw.tags as unknown[]).filter(
          (t): t is string => typeof t === "string" && t.length > 0,
        )
      : undefined;
    const skills = Array.isArray(matchRaw.skills)
      ? (matchRaw.skills as unknown[]).filter(
    ...
    const targetRaw =
      obj.target && typeof obj.target === "object" ? (obj.target as Record<string, unknown>) : null;
    if (!targetRaw) {
      continue;
    }
    ...
    const agentId =
      typeof targetRaw.agentId === "string" ? targetRaw.agentId.trim() || undefined : undefined;
