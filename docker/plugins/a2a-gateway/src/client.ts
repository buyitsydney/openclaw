...
export class A2AClient {
  ...
  private buildFactory(peer: PeerConfig): { factory: ClientFactory; path: string } {
    ...
    const authFetch = authHandler ? createAuthenticatingFetchWithRetry(fetch, authHandler) : fetch;
  ...
  private async doSendMessage(
    ...
    // ------------------------------------------------------------------
    const targetAgentId =
      typeof (message as any)?.agentId === "string" ? String((message as any).agentId) : "";
    ...
    try {
      ...
      const allInterfaces: TransportEndpoint[] = [{ url: agentCard.url, transport: mainTransport }];
    ...
    for (let i = 0; i < transports.length; i++) {
      ...
      try {
        const result = await this.doSendViaTransport(peer, endpoint, sendParams, requestOptions);
    ...
    const errorMessage =
      lastError instanceof Error
        ...
        : typeof lastError === "object" && lastError && "response" in lastError
          ? ((lastError as any).response?.error ?? String(lastError))
          : String(lastError);
