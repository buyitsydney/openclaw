...
export class QueueingAgentExecutor implements AgentExecutor {
  ...
  constructor(
    ...
    options: QueueingExecutorOptions,
  ) {
  ...
  private async runEntry(entry: QueuedTaskEntry): Promise<void> {
    ...
    const observedBus = createObservedEventBus(entry.eventBus, (event) => {
      const status =
        event.kind === "task" || event.kind === "status-update" ? event.status : undefined;
      if (!status) {
        return;
      }
      if (
        ...
        if (status.state !== "completed") {
          finalErrorMessage =
            typeof status.message?.parts?.[0] === "object" &&
            status.message.parts[0]?.kind === "text"
