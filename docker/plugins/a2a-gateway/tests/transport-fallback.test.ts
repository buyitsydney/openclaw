...
describe("orderTransports", () => {
  ...
  it("preserves order when only one transport exists", () => {
    const input: TransportEndpoint[] = [{ url: "http://x/jsonrpc", transport: "JSONRPC" }];
...
describe("isRetryableTransportError", () => {
  ...
  it("returns true for ECONNRESET", () => {
    assert.equal(isRetryableTransportError(new Error("read ECONNRESET")), true);
  });

  it("returns true for ETIMEDOUT", () => {
    assert.equal(isRetryableTransportError(new Error("connect ETIMEDOUT 10.0.0.1:443")), true);
  });

  it("returns true for fetch failed", () => {
    assert.equal(isRetryableTransportError(new Error("fetch failed")), true);
  });

  it("returns true for timeout errors", () => {
    assert.equal(isRetryableTransportError(new Error("The operation timed out")), true);
  });

  it("returns true for DNS / ENOTFOUND errors", () => {
    assert.equal(isRetryableTransportError(new Error("getaddrinfo ENOTFOUND example.com")), true);
  });

  it("returns true for gRPC UNAVAILABLE", () => {
    assert.equal(isRetryableTransportError(new Error("14 UNAVAILABLE: Connection refused")), true);
  });

  it("returns true for gRPC DEADLINE_EXCEEDED", () => {
    assert.equal(isRetryableTransportError(new Error("4 DEADLINE EXCEEDED")), true);
  });
  ...
  it("returns false for non-network Error", () => {
    assert.equal(isRetryableTransportError(new Error("Invalid argument")), false);
  });

  it("returns false for auth-related Error messages", () => {
    assert.equal(isRetryableTransportError(new Error("Authentication required")), false);
  });
...
describe("transport fallback scenario", () => {
  ...
  it("all transports fail → returns the last error", async () => {
    ...
    const errors = [new Error("ECONNREFUSED"), new Error("fetch failed"), new Error("UNAVAILABLE")];
