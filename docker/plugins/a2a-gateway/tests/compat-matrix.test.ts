      ...
      const harness = createHarness(
        makeConfig({
          peers: [
            {
              name: "minimal-peer",
              agentCardUrl: "http://mock-minimal/.well-known/agent-card.json",
            },
          ],
        }),
      );
      ...
      const harness = createHarness(
        makeConfig({
          peers: [
            {
              name: "extended-peer",
              agentCardUrl: "http://mock-extended/.well-known/agent-card.json",
            },
          ],
        }),
      );
  ...
  it("client handles skills with various formats (with/without tags, with/without id)", async () => {
    ...
    try {
      const harness = createHarness(
        makeConfig({
          peers: [
            { name: "skills-peer", agentCardUrl: "http://mock-skills/.well-known/agent-card.json" },
          ],
        }),
      );
  ...
  it("client handles Agent Card with no optional fields (no securitySchemes, no additionalInterfaces)", async () => {
    ...
    try {
      const harness = createHarness(
        makeConfig({
          peers: [
            { name: "bare-peer", agentCardUrl: "http://mock-bare/.well-known/agent-card.json" },
          ],
        }),
      );
        ...
        const harness = createHarness(
          makeConfig({
            peers: [
              {
                name: "version-peer",
                agentCardUrl: "http://mock-version/.well-known/agent-card.json",
              },
            ],
          }),
        );
    ...
    const fullCard = buildAgentCard(
      makeConfig({
        agentCard: {
          ...
          skills: [{ id: "s1", name: "chat", description: "Chat skill" }, "simple-skill"],
        },
...
describe("compat: inbound message format variations", () => {
  ...
  it("handles parts with extra unknown fields (forward compat)", async () => {
    ...
    try {
      ...
      assert.ok(
        capturedMessage.includes("forward compat test"),
        "should extract text despite extra fields",
      );
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
      ...
      const harness = createHarness(
        makeConfig({
          peers: [
            {
              name: "standard-peer",
              agentCardUrl: "http://mock-standard/.well-known/agent-card.json",
            },
          ],
        }),
      );
  ...
  it("handles JSON-RPC error response with standard code -32600", async () => {
    ...
    try {
      const harness = createHarness(
        makeConfig({
          peers: [
            { name: "error-peer", agentCardUrl: "http://mock-error/.well-known/agent-card.json" },
          ],
    ...
    try {
      const harness = createHarness(
        makeConfig({
          peers: [
            {
              name: "custom-err-peer",
              agentCardUrl: "http://mock-custom-err/.well-known/agent-card.json",
            },
          ],
          resilience: { retry: { maxRetries: 0, baseDelayMs: 0, maxDelayMs: 0 } },
        }),
  ...
  it("executor handles response with text + mediaUrls (mixed content)", async () => {
    ...
    try {
      ...
      assert.equal(
        fileParts.length,
        2,
        "should have two file parts (deduped from mediaUrl + mediaUrls)",
      );
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
  ...
  it("apiKey auth is sent via x-api-key header", async () => {
    ...
    try {
      ...
      assert.equal(
        ...
        "my-api-key-xyz",
        "x-api-key header should contain the token",
      );
    } finally {
      globalThis.fetch = originalFetch;
    }
        ...
        body: JSON.stringify({
          ...
          params: {
            message: {
              messageId: "m2",
              role: "user",
              parts: [{ kind: "text", text: "with auth" }],
            },
          },
        }),
        ...
        body: JSON.stringify({
          ...
          params: {
            message: {
              messageId: "m-raw",
              role: "user",
              parts: [{ kind: "text", text: "raw token" }],
            },
          },
        }),
        signal: AbortSignal.timeout(5_000),
