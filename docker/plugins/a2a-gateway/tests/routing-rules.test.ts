...
describe("parseRoutingRules", () => {
  ...
  it("drops entries with missing name", () => {
    const rules = parseRoutingRules([{ match: { pattern: "test" }, target: { peer: "Bot" } }]);
    assert.equal(rules.length, 0);
  });

  it("drops entries with missing match", () => {
    const rules = parseRoutingRules([{ name: "broken", target: { peer: "Bot" } }]);
    assert.equal(rules.length, 0);
  });

  it("drops entries with missing target", () => {
    const rules = parseRoutingRules([{ name: "broken", match: { pattern: "test" } }]);
    assert.equal(rules.length, 0);
  });

  it("drops entries with missing target.peer", () => {
    const rules = parseRoutingRules([{ name: "broken", match: { pattern: "test" }, target: {} }]);
    assert.equal(rules.length, 0);
  });

  it("drops entries with no match criteria", () => {
    const rules = parseRoutingRules([{ name: "empty-match", match: {}, target: { peer: "Bot" } }]);
    assert.equal(rules.length, 0);
  });
  ...
  it("filters out non-string tags and skills", () => {
    const rules = parseRoutingRules([
      {
        name: "mixed",
        match: { tags: ["valid", 42, null, "also-valid"] },
        target: { peer: "Bot" },
      },
    ]);
    ...
    const rules: RoutingRule[] = parseRoutingRules([
      {
        ...
        target: { peer: "Coder", agentId: "reviewer" },
      },
    ]);
