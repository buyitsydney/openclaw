...
describe("zero-config install (issue #7)", () => {
  ...
  it("builds Agent Card with defaults when agentCard fields are missing", () => {
    ...
    const card = buildAgentCard(minimalConfig as unknown as GatewayConfig) as Record<
      string,
      unknown
    >;
...
describe("session key format (PR #9, issue #8)", () => {
  it("session key uses agent: prefix for OpenClaw gateway compatibility", async () => {
    ...
    try {
      ...
      assert.ok(capturedSessionKey.includes("a2a:"), `session key should contain "a2a:" namespace`);
...
describe("a2a-gateway plugin", () => {
  it("builds an Agent Card with protocolVersion 0.3.0 and required fields", async () => {
    const payload = buildAgentCard(makeConfig() as unknown as GatewayConfig) as Record<
      string,
      unknown
    >;
  ...
  it("inbound FilePart (URI) is formatted as text for the agent", async () => {
    ...
    try {
      ...
      assert.ok(capturedMessage.includes("Check this image"), "should include the text part");
      assert.ok(
        capturedMessage.includes("https://example.com/photo.png"),
        "should include the file URI in the message",
      );
      assert.ok(capturedMessage.includes("photo.png"), "should include the filename");
      assert.ok(capturedMessage.includes("image/png"), "should include the MIME type");
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
  });
  ...
  it("inbound FilePart (base64) is formatted with size hint", async () => {
    ...
    try {
      ...
      assert.ok(capturedMessage.includes("doc.pdf"), "should include the filename");
      assert.ok(capturedMessage.includes("inline"), "should mention inline for base64 content");
      assert.ok(capturedMessage.includes("KB"), "should include size hint");
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
  });
  ...
  it("inbound DataPart is formatted as structured text for the agent", async () => {
    ...
    try {
      ...
      assert.ok(capturedMessage.includes("application/json"), "should include the mimeType");
      assert.ok(capturedMessage.includes("temperature"), "should include the data content");
      assert.ok(capturedMessage.includes("Beijing"), "should include nested data values");
      assert.ok(capturedMessage.includes("[Data"), "should use [Data prefix for DataPart");
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
  });
  ...
  it("inbound DataPart with primitive data value is formatted correctly", async () => {
    ...
    try {
      ...
      assert.ok(capturedMessage.includes("[1,2,3]"), "should include the array data");
      assert.ok(capturedMessage.includes("[Data"), "should use [Data prefix for DataPart");
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
  ...
  it("response with multiple mediaUrls produces multiple FileParts", async () => {
    ...
    try {
      ...
      const finalTask = published[published.length - 1] as Record<string, unknown>;
      const message = (finalTask.status as Record<string, unknown>).message as Record<
        string,
        unknown
      >;
      const parts = message.parts as Array<Record<string, unknown>>;
  ...
  it("a2a.send sends to mocked peer JSON-RPC endpoint", async () => {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        url === "http://mock-peer/.well-known/agent-card.json" ||
