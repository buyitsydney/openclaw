...
- **Testing in worktree — MUST-DO before container launch:** worktrees only contain git-tracked files. `docker/users.csv` and other gitignored configs are missing. Before starting any container from a worktree, symlink all required non-git files from the main repo:
  ```
