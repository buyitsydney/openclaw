...
async function listGroupBots(account: ResolvedFeishuAccount, chatId: string): Promise<BotEntry[]> {
...
const BotDirectorySchema = Type.Object({
  action: stringEnum(BOT_DIR_ACTIONS, {
    description:
      "search=fuzzy name search, list=paginated full list, group_bots=list bots in current group (force refresh)",
  }),
  query: Type.Optional(
    Type.String({ description: "Search query (name or app_id substring, for search action)" }),
  ),
  chat_id: Type.Optional(Type.String({ description: "Group chat ID (for group_bots action)" })),
  page: Type.Optional(
    Type.Number({ description: "Page number for list (default 1, 20 per page)" }),
  ),
});
