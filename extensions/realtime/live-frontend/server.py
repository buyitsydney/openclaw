...
RT_PORT = 18790  # Realtime server port (internal; Docker containers do NOT expose this externally)
...
# ---------------------------------------------------------------------------
# Voice token authentication (Layer 2 — identical logic for local and Docker)
# Token file: ~/.openclaw/.voice-token (persistent; survives restarts)
# Generated by /voice command (Feishu Bot) or start.sh (local convenience)
# ---------------------------------------------------------------------------
VOICE_TOKEN_PATH = os.path.join(
    os.environ.get("HOME", os.path.expanduser("~")), ".openclaw", ".voice-token"
)


def read_voice_token() -> str:
    """Read current voice token from file. Returns empty string if missing."""
    try:
        with open(VOICE_TOKEN_PATH) as f:
            return f.read().strip()
...
def check_token(request) -> bool:
    """Validate token query param against the token file."""
    expected = read_voice_token()
    if not expected:
        return False  # No token file = not initialized = reject all
    return request.query.get("token") == expected
...
# ---------------------------------------------------------------------------
# Realtime proxy routes — forward to internal RT server (port 18790)
# These routes allow external traffic to reach the realtime server through
# the FE proxy (port 8000), so the RT port never needs to be exposed.
# ---------------------------------------------------------------------------


async def proxy_bootstrap(request):
    """Proxy bootstrap API to internal realtime server with token validation."""
    if not check_token(request):
        return web.Response(status=401, text="Unauthorized")
    # Forward all query params (including token — server.ts validates independently)
    qs = request.query_string
    url = f"http://localhost:{RT_PORT}/api/realtime/bootstrap"
    if qs:
        url += f"?{qs}"
...
async def ws_realtime_proxy(request):
    """Proxy WebSocket to internal realtime server with token validation."""
    if not check_token(request):
        return web.Response(status=401, text="Unauthorized")
    ...
    try:
        ...
        ws_url = f"http://localhost:{RT_PORT}/ws"
        if token:
            ws_url += f"?token={token}"
        upstream = await session.ws_connect(ws_url)
    except Exception as e:
        print(f"Failed to connect to realtime server: {e}")
        await ws_resp.close(code=1011, message=b"Upstream connection failed")
        await session.close()
        return ws_resp
...
async def start_http_server():
    ...
    # Realtime proxy routes (must be before static file catch-all)
    app.router.add_get("/api/realtime/bootstrap", proxy_bootstrap)
    app.router.add_get("/ws", ws_realtime_proxy)
